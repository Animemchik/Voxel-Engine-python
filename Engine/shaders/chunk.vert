...
void unpack(uint packed_data) {
    ...
    uint b_bit = 5u, c_bit = 5u, d_bit = 11u, e_bit = 3u, f_bit = 2u, g_bit = 1u;
    uint b_mask = 31u, c_mask = 31u, d_mask = 2047u, e_mask = 7u, f_mask = 3u, g_mask = 1u;
    //
...
void main() {
    ...
    vec3 frag_world_pos = (matrix_model * vec4(in_position, 1.0)).xyz;

    gl_Position = matrix_projection * matrix_view * vec4(frag_world_pos, 1.0);
}